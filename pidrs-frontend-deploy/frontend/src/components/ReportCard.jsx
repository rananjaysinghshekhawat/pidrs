...
const resolveImage = (path) => (path && path.startsWith("http") ? path : `${backendOrigin}${path}`);

const ReportCard = ({ report, isAuthority, onStatusChange, onDelete }) => {
  return (
    <div className="ticket flex flex-col overflow-hidden">
      {report.imageUrl && (
        <img src={resolveImage(report.imageUrl)} alt={report.title} className="w-full h-40 object-cover" />
      )}
